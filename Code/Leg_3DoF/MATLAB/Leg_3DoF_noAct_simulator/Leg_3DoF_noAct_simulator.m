classdef Leg_3DoF_noAct_simulator < handle
    % Leg_3DoF_ACA_simulator - Simulator class for the 3-DoF leg, undriven
    % Comment 'Check joint limits' in Leg_3DoF in order to execute
    
    
    %__________________________________________________________________
    % Properties
    properties
       model        % Leg_3DoF_noAct model
    ...
    methods
        ...
        %__________________________________________________________________
        % Constructor
        function this = Leg_3DoF_noAct_simulator(legParamsFileName)
            ...
            % Get object model
            this.model =  Leg_3DoF_noAct( legParamsFileName);
            
            % Simulation parameters
            this.params.tspan       = [0 5];                   % Simulation time [s]
        ...
        function [] = run(this, interactive)
         ...
         toc
            ...
            disp('Reprocessing states to obtain outputs. No inputs and references..');
            tic
            
            % Preallocate variables
            y       = zeros(length(t), this.model.leg.K);
            u       = zeros(length(t), this.model.leg.M);
            
            for i=1:length(t)
                [ ~, y(i,:), u(i,:) ]     = this.model.dx(t(i),x(i,:)');
            ...
            % Get the leg states
            x_leg	= x';%x(:,this.model.leg.N)';

            % Get the leg outputs
            y_leg	= y(:,this.model.leg.K)';

            % Get the leg inputs
            u_leg	= u(:,this.model.leg.M)'; 
...
%         %__________________________________________________________________
        % Animate the leg
        function [] = animate(this, t, x_leg, infoText) % tau removed as input
            % Default parameters
