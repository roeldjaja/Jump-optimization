...
%             this.setInitialStates(q_0, q_d_0);
        ...
        function [ q_ref, q_d_ref ] = q_ref( this, t )
            ...
            % Cartesian reference
            x_hip       = this.ref.x_hip_bias + ...
                            this.ref.x_hip_amp * sin(this.ref.x_hip_freq * t + this.ref.x_hip_phase);
            y_hip       = this.ref.y_hip_bias + ...
                            this.ref.y_hip_amp * sin(this.ref.y_hip_freq * t + this.ref.y_hip_phase);
...
%             Squat initials
%            q_0     = [0; 0; 0; -0.6; 1.5; -1.2]; % Leg starts slightly in the air
%             q_d_0   = [0; 0; 0; 0; 0; 0];
%             this.setInitialStates(q_0, q_d_0);
            ...
            if (0<=t) & (t<=stg1)                    
             ...
             % Stage 1: Hip, knee and ankle extension
           ...
           % Stage 2:  push off
            if (stg1+stg2+stg3<t) %& (t<=(stg1+stg2+stg3+stg4))
                    q_ref(2,:)  = 0;
                    q_ref(3,:)  = 0;
                    q_ref(4,:)  = -0.2;
                    q_ref(5,:)  = 0.8; %0.6
            ...
            dxdt        = this.ref.x_hip_amp * this.ref.x_hip_freq * ...
                            cos(this.ref.x_hip_freq * t + this.ref.x_hip_phase);
            dydt        = this.ref.y_hip_amp * this.ref.x_hip_freq * ...
                            cos(this.ref.y_hip_freq * t + this.ref.y_hip_phase);
            ...
            dbdx        = -1 ./ (y + (x.^2)./y);
            dbdy        = x ./ (x.^2 + y.^2);
            dq4dr       = 1 ./ (this.leg.params.l2 * sqrt(4 - (r.^2 ./ this.leg.params.l2^2)));
            dq4db       = 1;
            dq5dr       = -2 ./ (this.leg.params.l2 * sqrt(4 - (r.^2 ./ this.leg.params.l2^2)));
            dq5db       = 0;
            ...
            q_d_ref(2,:)    = 0;
            q_d_ref(3,:)    = 0;
            q_d_ref(4,:)    = dq4dr .* drdt + dq4db .* dbdt;
            q_d_ref(5,:)    = dq5dr .* drdt + dq5db .* dbdt;
            q_d_ref(6,:)    = -(q_d_ref(4,:) + q_d_ref(5,:));
        end
