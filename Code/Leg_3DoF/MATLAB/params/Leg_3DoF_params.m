% [ params ] = Leg_3DoF_params()
% Parameter file for the Leg_3DoF package
function [ params ] = Leg_3DoF_params()

    % Define constants and parameters

    % Name for this set of parameters
    params.name     = 'Leg_3DoF_params';

    % Constants
    params.g        = 9.81;             % Gravitational constant [m s^-2]

    % Dimensions
    params.l1       = 0.26;             % Link 1 length [m]
    params.l2       = 0.35;             % Link 2 length [m]
    params.l3       = 0.35;             % Link 3 length [m]
    params.l4       = 0.30;             % Link 4 length [m]
    params.r_toe    = 0.16;             % Foot CoM to toe [m]
    params.r_heel   = params.l1 - params.r_toe; % Foot CoM to heel [m]
    
    % Joint locations
    params.r1       = 0.03;             % Joint location on link 1 [m]
    params.r2       = params.l2/2;    	% Joint location on link 2 [m]
    params.r3       = params.l3/2;   	% Joint location on link 3 [m]
    params.r4       = params.l4;    	% Joint location on link 4 [m]

    % Masses/inertiae
    params.m1       = 2.50;             % Link 1 mass [kg]
    params.J1       = params.m1 * params.l1^2 / 12; % Link 1 rot. inertia [kg m^2] (cilinder around midpoint)
    
    params.m2       = 4.41;             % Link 2 mass [kg] (link + motors)
    params.J2       = params.m2 * params.l2^2 / 12; % Link 2 rot. inertia [kg m^2] (cilinder around midpoint)

    params.m3       = 5.414;            % Link 3 mass [kg] (link + motors)
    params.J3       = params.m3 * params.l3^2 / 12; % Link 3 rot. inertia [kg m^2] (cilinder around midpoint)
