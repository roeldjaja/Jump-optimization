...
The designs of the actuation configurations have been optimised to select actuation
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=\linewidth]{cad}
...
\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.15]{Leg_3DoF_model}
	\caption{Leg model: The straight configuration on
		the right corresponds to q = 0}
...
As described in Section \ref{sec:legDesign} different configurations of \textit{ACA} are used to actuate the leg. Motor M2, shown in Fig. \ref{fig:ACA}, can be used to apply pretension on the \textit{ESB} elastic element. Let $r_p$ denote the pulley radius and let $p$ denote a certain pretension of motor M2. With a corresponding joint configuration $q$, the elongation of each \textit{ESB} elastic element can be described as $\Delta_p = p - r_p q$. Note that the element with stiffness $k_p$ and damping $d_p$ only applies elastic force under extension as it is of a rubber type material:
\begin{equation}
F_p =	\left\lbrace
\begin{array}{ll}
...
\label{eq:F_p}
\end{equation}   
The \textit{ESB} generates a torque on the joint equal to
\begin{equation}
\tau_p = r_p \, F_p.
\label{eq:tau_p_mono}
\end{equation}
For the biarticulated configuration shown in Fig. \ref{fig:ACA} the driven joint can be expressed as joint $i$ and the spanned joint can be expressed as joint $j$. With pulley radii $r_{p,i}$ and $r_{p,j}$, the elongation of the \textit{ESB} elastic element is then given by:
\begin{equation}
\Delta_p = p - r_{p,i} \, q_i - r_{p,j} \, q_j,
\label{eq:Delta_p_biart}
\end{equation}
with the \textit{ESB} applying a torque to each joint 
\begin{equation}
\tau_i = r_{p,i} \, F_p, \hspace{5mm} \tau_j = r_{p,j} \, F_p.
\label{eq:tau_p_bi}
\end{equation} 
...
As mentioned in Section \ref{subsec:actuationModel}, the series elastic element in the \textit{ACA} scheme can be set up with a pretension with motor M2, shown in Fig. \ref{fig:ACA}. The elastic element pretension position is an interesting parameter to include in the optimisation as different pretensions will yield different jumping behaviour. Therefore, for the mono- and biarticulated configurations, the pretension position $p$, the position of the elastic element attachment relative to its undeformed attachment position, is taken into account as an optimisation variable.
...
\textbf{TODO: Ended review (for now) here..}
...
The optimisation has been performed with MATLAB's \texttt{fmincon} for the upward movement of the leg performing a jumping motion with a maximum duration of 0.4 seconds. The initial position of the leg yields a squatting posture and the optimisation is concluded when the CoM reaches its highest point for successful jumps or when the leg has fallen over for unsuccessful jumps. The initial guess is a manually constructed motion as shown in Fig. \ref{fig:seq} for actuation without energy storage branches. The optimisation results for the objective presented in Section \ref{sec:dynamicOptimisation} are shown in Table \ref{maxheight}. For the three actuation configurations, Fig. \ref{fig:opt_results} displays the objective criterion values during the optimisation, the joint torques, their net power and the \textit{ESB} tendon energy storage. The values of the used scaling constants described in Section \ref{subsec:objectiveCriteria} yield: $c_1=5 \cdot 10^{2} \,, c_2=1.5 \cdot 10^{-4} \,, c_3=1.5 \cdot 10^{3} \,, c_4 = 1\cdot 10^{2} \,, \text{and} \, c_5 = 1$. 
The video corresponding to this paper shows a visualization of the optimised jumping motion on all three actuation configurations.
...
\begin{table*}[ht]
	\caption{Jumping optimisation results for different configurations}
	\label{maxheight}
	\begin{center}
